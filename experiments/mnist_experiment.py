...
if __name__ == "__main__":
    ...
    # sweep_id = wandb.sweep(entity="peter-blohm-tu-wien", project="pag_mnist_scale_normal",
    #                        sweep=sweep_configuration)
    # wandb.agent(sweep_id, function=lambda: temperature_scale_network("mnist", "feed_forward"))
    sweep_id = wandb.sweep(entity="peter-blohm-tu-wien", project="pag_mnist_sample_normal",
                           sweep=sweep_configuration)
    wandb.agent(sweep_id, function=lambda: sampling("mnist", "feed_forward", "pgd"))
    sweep_id = wandb.sweep(entity="peter-blohm-tu-wien", project="pag_mnist_test_normal",
                           sweep=sweep_configuration)
